...
android {
    compileSdk = 36
    namespace = "com.nickdu.projex"
    defaultConfig {
        manifestPlaceholders["usesCleartextTraffic"] = "false"
        applicationId = "com.nickdu.projex"
        minSdk = 24
        targetSdk = 36
    ...
    // Local release builds can use a local keystore file (see ANDROID_DEV_SETUP.md).
    signingConfigs {
        create("release") {
    ...
    buildTypes {
        ...
        getByName("release") {
            // Use release signing config when credentials are available; fall back to
            // debug signing for local builds without a keystore configured.
...
dependencies {
    ...
    // WorkManager for periodic background sync (≥15 min)
